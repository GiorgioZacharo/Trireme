...
PARALLEL_LOOP=0
	...
	#for  (( i=1; i<=5; i++ )); do 
        	if [[ $FUNC_NAME == ${TASK[$i]} ]]; then
                  ...
                  # for (( LLP_FACTOR=2; $LLP_FACTOR<=$LLP_MAX;  )); do
                        ...
                        printf "${TASK[$i]}-$LLP_FACTOR\t" >> parallel_tasks_tlp-llp.txt
			PARALLEL_LOOP=1
		##else

			##printf "${TASK[$i]} " >> parallel_tasks_tlp-llp.txt
 ...
 #printf "\n" >> parallel_tasks_tlp-llp.txt
  ...
  # done <parallel_tasks_filtered.txt 
    done <parallel_tasks.txt 

cat  parallel_tasks_tlp-llp.txt
